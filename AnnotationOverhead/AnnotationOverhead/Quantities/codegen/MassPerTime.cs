...
#region units
	private static Unit_ kilogrampersecond_ = new Unit_("KilogramPerSecond", "kg/s", 1.0);
...
#region comparisons
    ...
    public bool Equals(MassPerTime other) => other is object && Equals((object)other);
...
#region conversions
		public static MassPerTime FromKilogramPerSeconds(double value, double minValue, double maxValue) => new MassPerTime(value * kilogrampersecond_.Factor, minValue * kilogrampersecond_.Factor, maxValue * kilogrampersecond_.Factor);
		public static MassPerTime FromKilogramPerSeconds(double value) => new MassPerTime(value * kilogrampersecond_.Factor);
		public double GetValueInKilogramPerSeconds() => Value_ / kilogrampersecond_.Factor;
...
#region rules
    ...
    public static bool ValidateRules(MassPerTime masspertime, bool throwOnFail=true)
    {
        bool fail = false;
        if (masspertime.HasMinValue && masspertime.Value_ < masspertime.MinValue)
        {
            fail = true;
        }

        if (masspertime.HasMaxValue && masspertime.Value_ > masspertime.MaxValue)
        {
            fail = true;
        }
        ...
        if (fail && throwOnFail)
        {
            new System.ArgumentException("MassPerTime is outside its bounds.");
        }
        else if (fail)
        {
            return false;
