...
#region units
	private static Unit_ persecondcub_ = new Unit_("PerSecondCub", "/s³", 1.0);
...
#region comparisons
    ...
    public bool Equals(PerTime other) => other is object && Equals((object)other);
...
#region conversions
		public static PerTime FromPerSecondCubs(double value, double minValue, double maxValue) => new PerTime(value * persecondcub_.Factor, minValue * persecondcub_.Factor, maxValue * persecondcub_.Factor);
		public static PerTime FromPerSecondCubs(double value) => new PerTime(value * persecondcub_.Factor);
		public double GetValueInPerSecondCubs() => Value_ / persecondcub_.Factor;
...
#region rules
    ...
    public static bool ValidateRules(PerTime pertime, bool throwOnFail=true)
    {
        bool fail = false;
        if (pertime.HasMinValue && pertime.Value_ < pertime.MinValue)
        {
            fail = true;
        }

        if (pertime.HasMaxValue && pertime.Value_ > pertime.MaxValue)
        {
            fail = true;
        }
        ...
        if (fail && throwOnFail)
        {
            new System.ArgumentException("PerTime is outside its bounds.");
        }
        else if (fail)
        {
            return false;
