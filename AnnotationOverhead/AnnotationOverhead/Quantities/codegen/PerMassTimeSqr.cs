...
namespace RaySearch.CorePlatform.Framework
{
    ...
    private static void ImportDisplayUnitFromDatabase_()
    {
        if (!displayUnitIsImported_ && FakeDatabase.Units.ContainsKey("PerMassTimeSqr"))
        {
            ...
            {
				      case "perkilogramcubsecondsqr": displayUnit_ = perkilogramcubsecondsqr_; break;
...
#region units
	private static Unit_ perkilogramcubsecondsqr_ = new Unit_("PerKilogramCubSecondSqr", "/kg³ s²", 1.0);
	private static Unit_ platformUnit_ = perkilogramcubsecondsqr_;
	private static Unit_ displayUnit_ = perkilogramcubsecondsqr_;
#endregion
...
#region comparisons
    ...
    public bool Equals(PerMassTimeSqr other) => other is object && Equals((object)other);
...
#region conversions
		public static PerMassTimeSqr FromPerKilogramCubSecondSqrs(double value, double minValue, double maxValue) => new PerMassTimeSqr(value * perkilogramcubsecondsqr_.Factor, minValue * perkilogramcubsecondsqr_.Factor, maxValue * perkilogramcubsecondsqr_.Factor);
		public static PerMassTimeSqr FromPerKilogramCubSecondSqrs(double value) => new PerMassTimeSqr(value * perkilogramcubsecondsqr_.Factor);
		public double GetValueInPerKilogramCubSecondSqrs() => Value_ / perkilogramcubsecondsqr_.Factor;
...
#region rules
    ...
    public static bool ValidateRules(PerMassTimeSqr permasstimesqr, bool throwOnFail=true)
    {
        bool fail = false;
        if (permasstimesqr.HasMinValue && permasstimesqr.Value_ < permasstimesqr.MinValue)
        {
            fail = true;
        }

        if (permasstimesqr.HasMaxValue && permasstimesqr.Value_ > permasstimesqr.MaxValue)
        {
            fail = true;
        }
        ...
        if (fail && throwOnFail)
        {
            new System.ArgumentException("PerMassTimeSqr is outside its bounds.");
        }
        else if (fail)
        {
            return false;
