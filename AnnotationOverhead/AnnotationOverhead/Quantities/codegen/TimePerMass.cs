...
#region units
	private static Unit_ secondperkilogram_ = new Unit_("SecondPerKilogram", "s/kg", 1.0);
...
#region comparisons
    ...
    public bool Equals(TimePerMass other) => other is object && Equals((object)other);
...
#region conversions
		public static TimePerMass FromSecondPerKilograms(double value, double minValue, double maxValue) => new TimePerMass(value * secondperkilogram_.Factor, minValue * secondperkilogram_.Factor, maxValue * secondperkilogram_.Factor);
		public static TimePerMass FromSecondPerKilograms(double value) => new TimePerMass(value * secondperkilogram_.Factor);
		public double GetValueInSecondPerKilograms() => Value_ / secondperkilogram_.Factor;
...
#region rules
    ...
    public static bool ValidateRules(TimePerMass timepermass, bool throwOnFail=true)
    {
        bool fail = false;
        if (timepermass.HasMinValue && timepermass.Value_ < timepermass.MinValue)
        {
            fail = true;
        }

        if (timepermass.HasMaxValue && timepermass.Value_ > timepermass.MaxValue)
        {
            fail = true;
        }
        ...
        if (fail && throwOnFail)
        {
            new System.ArgumentException("TimePerMass is outside its bounds.");
        }
        else if (fail)
        {
            return false;
