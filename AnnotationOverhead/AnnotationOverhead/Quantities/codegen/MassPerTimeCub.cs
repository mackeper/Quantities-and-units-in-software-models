...
#region units
	private static Unit_ kilogrampersecondcub_ = new Unit_("KilogramPerSecondCub", "kg/s³", 1.0);
...
#region comparisons
    ...
    public bool Equals(MassPerTimeCub other) => other is object && Equals((object)other);
...
#region conversions
		public static MassPerTimeCub FromKilogramPerSecondCubs(double value, double minValue, double maxValue) => new MassPerTimeCub(value * kilogrampersecondcub_.Factor, minValue * kilogrampersecondcub_.Factor, maxValue * kilogrampersecondcub_.Factor);
		public static MassPerTimeCub FromKilogramPerSecondCubs(double value) => new MassPerTimeCub(value * kilogrampersecondcub_.Factor);
		public double GetValueInKilogramPerSecondCubs() => Value_ / kilogrampersecondcub_.Factor;
...
#region rules
    ...
    public static bool ValidateRules(MassPerTimeCub masspertimecub, bool throwOnFail=true)
    {
        bool fail = false;
        if (masspertimecub.HasMinValue && masspertimecub.Value_ < masspertimecub.MinValue)
        {
            fail = true;
        }

        if (masspertimecub.HasMaxValue && masspertimecub.Value_ > masspertimecub.MaxValue)
        {
            fail = true;
        }
        ...
        if (fail && throwOnFail)
        {
            new System.ArgumentException("MassPerTimeCub is outside its bounds.");
        }
        else if (fail)
        {
            return false;
