...
#region units
	private static Unit_ secondcubkilogram_ = new Unit_("SecondCubKilogram", "s³ kg", 1);
...
#region comparisons
    ...
    public bool Equals(TimeCubMass other) => other is object && Equals((object)other);
...
#region conversions
		public static TimeCubMass FromSecondCubKilograms(double value, double minValue, double maxValue) => new TimeCubMass(value * secondcubkilogram_.Factor, minValue * secondcubkilogram_.Factor, maxValue * secondcubkilogram_.Factor);
		public static TimeCubMass FromSecondCubKilograms(double value) => new TimeCubMass(value * secondcubkilogram_.Factor);
		public double GetValueInSecondCubKilograms() => Value_ / secondcubkilogram_.Factor;
...
#region rules
    ...
    public static bool ValidateRules(TimeCubMass timecubmass, bool throwOnFail=true)
    {
        bool fail = false;
        if (timecubmass.HasMinValue && timecubmass.Value_ < timecubmass.MinValue)
        {
            fail = true;
        }

        if (timecubmass.HasMaxValue && timecubmass.Value_ > timecubmass.MaxValue)
        {
            fail = true;
        }
        ...
        if (fail && throwOnFail)
        {
            new System.ArgumentException("TimeCubMass is outside its bounds.");
        }
        else if (fail)
        {
            return false;
