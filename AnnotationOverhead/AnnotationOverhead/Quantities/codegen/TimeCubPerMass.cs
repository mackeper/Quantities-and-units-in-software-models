...
#region units
	private static Unit_ secondcubperkilogram_ = new Unit_("SecondCubPerKilogram", "s³/kg", 1.0);
...
#region comparisons
    ...
    public bool Equals(TimeCubPerMass other) => other is object && Equals((object)other);
...
#region conversions
		public static TimeCubPerMass FromSecondCubPerKilograms(double value, double minValue, double maxValue) => new TimeCubPerMass(value * secondcubperkilogram_.Factor, minValue * secondcubperkilogram_.Factor, maxValue * secondcubperkilogram_.Factor);
		public static TimeCubPerMass FromSecondCubPerKilograms(double value) => new TimeCubPerMass(value * secondcubperkilogram_.Factor);
		public double GetValueInSecondCubPerKilograms() => Value_ / secondcubperkilogram_.Factor;
...
#region rules
    ...
    public static bool ValidateRules(TimeCubPerMass timecubpermass, bool throwOnFail=true)
    {
        bool fail = false;
        if (timecubpermass.HasMinValue && timecubpermass.Value_ < timecubpermass.MinValue)
        {
            fail = true;
        }

        if (timecubpermass.HasMaxValue && timecubpermass.Value_ > timecubpermass.MaxValue)
        {
            fail = true;
        }
        ...
        if (fail && throwOnFail)
        {
            new System.ArgumentException("TimeCubPerMass is outside its bounds.");
        }
        else if (fail)
        {
            return false;
