...
namespace RaySearch.CorePlatform.Framework
{
    ...
    [Serializable]
    public class Length : Quantity, IEquatable<Length>, IComparable
    {
...
#region units
	private static Unit_ centimeter_ = new Unit_("Centimeter", "cm", 1);
	private static Unit_ millimeter_ = new Unit_("Millimeter", "mm", 0.1);
	private static Unit_ meter_ = new Unit_("Meter", "m", 100);
	private static Unit_ inch_ = new Unit_("Inch", "in", 2.56);
...
#region comparisons
    ...
    public bool Equals(Length other) => other is object && Equals((object)other);
...
#region conversions
		public static Length FromCentimeters(double value, double minValue, double maxValue) => new Length(value * centimeter_.Factor, minValue * centimeter_.Factor, maxValue * centimeter_.Factor);
		public static Length FromCentimeters(double value) => new Length(value * centimeter_.Factor);
		public double GetValueInCentimeters() => Value_ / centimeter_.Factor;
		public static Length FromMillimeters(double value, double minValue, double maxValue) => new Length(value * millimeter_.Factor, minValue * millimeter_.Factor, maxValue * millimeter_.Factor);
		public static Length FromMillimeters(double value) => new Length(value * millimeter_.Factor);
		public double GetValueInMillimeters() => Value_ / millimeter_.Factor;
		public static Length FromMeters(double value, double minValue, double maxValue) => new Length(value * meter_.Factor, minValue * meter_.Factor, maxValue * meter_.Factor);
		public static Length FromMeters(double value) => new Length(value * meter_.Factor);
		public double GetValueInMeters() => Value_ / meter_.Factor;
		public static Length FromInchs(double value, double minValue, double maxValue) => new Length(value * inch_.Factor, minValue * inch_.Factor, maxValue * inch_.Factor);
		public static Length FromInchs(double value) => new Length(value * inch_.Factor);
		public double GetValueInInchs() => Value_ / inch_.Factor;
...
#region rules
    ...
    public static bool ValidateRules(Length length, bool throwOnFail=true)
    {
        bool fail = false;
        if (length.HasMinValue && length.Value_ < length.MinValue)
        {
            fail = true;
        }

        if (length.HasMaxValue && length.Value_ > length.MaxValue)
        {
            fail = true;
        }
        ...
        if (fail && throwOnFail)
        {
            new System.ArgumentException("Length is outside its bounds.");
        }
        else if (fail)
        {
            return false;
