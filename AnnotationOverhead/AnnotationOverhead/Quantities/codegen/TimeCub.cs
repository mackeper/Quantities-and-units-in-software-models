...
#region units
	private static Unit_ secondcub_ = new Unit_("SecondCub", "s³", 1);
...
#region comparisons
    ...
    public bool Equals(TimeCub other) => other is object && Equals((object)other);
...
#region conversions
		public static TimeCub FromSecondCubs(double value, double minValue, double maxValue) => new TimeCub(value * secondcub_.Factor, minValue * secondcub_.Factor, maxValue * secondcub_.Factor);
		public static TimeCub FromSecondCubs(double value) => new TimeCub(value * secondcub_.Factor);
		public double GetValueInSecondCubs() => Value_ / secondcub_.Factor;
...
#region rules
    ...
    public static bool ValidateRules(TimeCub timecub, bool throwOnFail=true)
    {
        bool fail = false;
        if (timecub.HasMinValue && timecub.Value_ < timecub.MinValue)
        {
            fail = true;
        }

        if (timecub.HasMaxValue && timecub.Value_ > timecub.MaxValue)
        {
            fail = true;
        }
        ...
        if (fail && throwOnFail)
        {
            new System.ArgumentException("TimeCub is outside its bounds.");
        }
        else if (fail)
        {
            return false;
