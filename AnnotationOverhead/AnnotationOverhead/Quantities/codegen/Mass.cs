...
namespace RaySearch.CorePlatform.Framework
{
    ...
    [Serializable]
    public class Mass : Quantity, IEquatable<Mass>, IComparable
    {
...
#region units
	private static Unit_ kilogram_ = new Unit_("Kilogram", "kg", 1);
...
#region comparisons
    ...
    public bool Equals(Mass other) => other is object && Equals((object)other);
...
#region conversions
		public static Mass FromKilograms(double value, double minValue, double maxValue) => new Mass(value * kilogram_.Factor, minValue * kilogram_.Factor, maxValue * kilogram_.Factor);
		public static Mass FromKilograms(double value) => new Mass(value * kilogram_.Factor);
		public double GetValueInKilograms() => Value_ / kilogram_.Factor;
...
#region rules
    ...
    public static bool ValidateRules(Mass mass, bool throwOnFail=true)
    {
        bool fail = false;
        if (mass.HasMinValue && mass.Value_ < mass.MinValue)
        {
            fail = true;
        }

        if (mass.HasMaxValue && mass.Value_ > mass.MaxValue)
        {
            fail = true;
        }
        ...
        if (fail && throwOnFail)
        {
            new System.ArgumentException("Mass is outside its bounds.");
        }
        else if (fail)
        {
            return false;
