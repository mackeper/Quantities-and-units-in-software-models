...
namespace RaySearch.CorePlatform.Framework
{
    ...
    private static void ImportDisplayUnitFromDatabase_()
    {
        if (!displayUnitIsImported_ && FakeDatabase.Units.ContainsKey("PerMass"))
        {
            ...
            {
				      case "perkilogramcub": displayUnit_ = perkilogramcub_; break;
...
#region units
	private static Unit_ perkilogramcub_ = new Unit_("PerKilogramCub", "/kg³", 1.0);
	private static Unit_ platformUnit_ = perkilogramcub_;
	private static Unit_ displayUnit_ = perkilogramcub_;
#endregion
...
#region comparisons
    ...
    public bool Equals(PerMass other) => other is object && Equals((object)other);
...
#region conversions
		public static PerMass FromPerKilogramCubs(double value, double minValue, double maxValue) => new PerMass(value * perkilogramcub_.Factor, minValue * perkilogramcub_.Factor, maxValue * perkilogramcub_.Factor);
		public static PerMass FromPerKilogramCubs(double value) => new PerMass(value * perkilogramcub_.Factor);
		public double GetValueInPerKilogramCubs() => Value_ / perkilogramcub_.Factor;
...
#region rules
    ...
    public static bool ValidateRules(PerMass permass, bool throwOnFail=true)
    {
        bool fail = false;
        if (permass.HasMinValue && permass.Value_ < permass.MinValue)
        {
            fail = true;
        }

        if (permass.HasMaxValue && permass.Value_ > permass.MaxValue)
        {
            fail = true;
        }
        ...
        if (fail && throwOnFail)
        {
            new System.ArgumentException("PerMass is outside its bounds.");
        }
        else if (fail)
        {
            return false;
