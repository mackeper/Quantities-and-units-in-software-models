...
namespace RaySearch.CorePlatform.Framework
{
    ...
    private static void ImportDisplayUnitFromDatabase_()
    {
        if (!displayUnitIsImported_ && FakeDatabase.Units.ContainsKey("PerMassTimeCub"))
        {
            ...
            {
				      case "perkilogramcubsecondcub": displayUnit_ = perkilogramcubsecondcub_; break;
...
#region units
	private static Unit_ perkilogramcubsecondcub_ = new Unit_("PerKilogramCubSecondCub", "/kg³ s³", 1.0);
	private static Unit_ platformUnit_ = perkilogramcubsecondcub_;
	private static Unit_ displayUnit_ = perkilogramcubsecondcub_;
#endregion
...
#region comparisons
    ...
    public bool Equals(PerMassTimeCub other) => other is object && Equals((object)other);
...
#region conversions
		public static PerMassTimeCub FromPerKilogramCubSecondCubs(double value, double minValue, double maxValue) => new PerMassTimeCub(value * perkilogramcubsecondcub_.Factor, minValue * perkilogramcubsecondcub_.Factor, maxValue * perkilogramcubsecondcub_.Factor);
		public static PerMassTimeCub FromPerKilogramCubSecondCubs(double value) => new PerMassTimeCub(value * perkilogramcubsecondcub_.Factor);
		public double GetValueInPerKilogramCubSecondCubs() => Value_ / perkilogramcubsecondcub_.Factor;
...
#region rules
    ...
    public static bool ValidateRules(PerMassTimeCub permasstimecub, bool throwOnFail=true)
    {
        bool fail = false;
        if (permasstimecub.HasMinValue && permasstimecub.Value_ < permasstimecub.MinValue)
        {
            fail = true;
        }

        if (permasstimecub.HasMaxValue && permasstimecub.Value_ > permasstimecub.MaxValue)
        {
            fail = true;
        }
        ...
        if (fail && throwOnFail)
        {
            new System.ArgumentException("PerMassTimeCub is outside its bounds.");
        }
        else if (fail)
        {
            return false;
