...
#region units
  private static Unit_ persecondsqr_ = new Unit_("PerSecondSqr", "/s²", 1.0, 0);
...
#region comparisons
  ...
  public override bool Equals(Object obj) => obj is PerTimeSqr other && Equals(other);
  public bool Equals(PerTimeSqr other) => other is object && Math.Abs(Value_ - other.Value_) <= epsilon;
...
#region conversions
  public static PerTimeSqr FromPerSecondSqrs(double value, double minValue, double maxValue) => new PerTimeSqr((value + persecondsqr_.Offset) * persecondsqr_.Factor, minValue * persecondsqr_.Factor, maxValue * persecondsqr_.Factor);
  public static PerTimeSqr FromPerSecondSqrs(double value) => new PerTimeSqr((value + persecondsqr_.Offset) * persecondsqr_.Factor);
  public double GetValueInPerSecondSqrs() => (Value_ - persecondsqr_.Offset) / persecondsqr_.Factor;
...
#region rules
  ...
  public bool ValidateRules(bool throwOnFail=true)
  {
    ...
    if (fail && throwOnFail)
    {
      throw new System.ArgumentException("PerTimeSqr is outside its bounds.");
    }
    else if (fail)
    {
      return false;
