...
#region units
  private static Unit_ secondsqr_ = new Unit_("SecondSqr", "s²", 1, 0);
...
#region comparisons
  ...
  public override bool Equals(Object obj) => obj is TimeSqr other && Equals(other);
  public bool Equals(TimeSqr other) => other is object && Math.Abs(Value_ - other.Value_) <= epsilon;
...
#region conversions
  public static TimeSqr FromSecondSqrs(double value, double minValue, double maxValue) => new TimeSqr((value + secondsqr_.Offset) * secondsqr_.Factor, minValue * secondsqr_.Factor, maxValue * secondsqr_.Factor);
  public static TimeSqr FromSecondSqrs(double value) => new TimeSqr((value + secondsqr_.Offset) * secondsqr_.Factor);
  public double GetValueInSecondSqrs() => (Value_ - secondsqr_.Offset) / secondsqr_.Factor;
...
#region rules
  ...
  public bool ValidateRules(bool throwOnFail=true)
  {
    ...
    if (fail && throwOnFail)
    {
      throw new System.ArgumentException("TimeSqr is outside its bounds.");
    }
    else if (fail)
    {
      return false;
