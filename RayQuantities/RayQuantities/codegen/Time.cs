...
#region units
  private static Unit_ second_ = new Unit_("Second", "s", 1, 0);
...
#region comparisons
  ...
  public bool Equals(Time other) => other is object && Math.Abs(Value_ - other.Value_) <= epsilon;
...
#region conversions
  public static Time FromSeconds(double value, double minValue, double maxValue) => new Time((value + second_.Offset) * second_.Factor, minValue * second_.Factor, maxValue * second_.Factor);
  public static Time FromSeconds(double value) => new Time((value + second_.Offset) * second_.Factor);
  public double GetValueInSeconds() => (Value_ - second_.Offset) / second_.Factor;
...
#region rules
  ...
  public bool ValidateRules(bool throwOnFail=true)
  {
    ...
    if (fail && throwOnFail)
    {
      throw new System.ArgumentException("Time is outside its bounds.");
    }
    else if (fail)
    {
      return false;
