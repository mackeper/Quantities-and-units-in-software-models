...
namespace RayQuantities
{
  ...
  {  
    ...
    if (!displayUnitIsImported_ && FakeDatabase.Units.ContainsKey("PerMassTime"))
    {
      ...
      {
        case "perkilogramsecond": displayUnit_ = perkilogramsecond_; break;
...
#region units
  private static Unit_ perkilogramsecond_ = new Unit_("PerKilogramSecond", "/kg s", 1.0, 0);
  private static Unit_ platformUnit_ = perkilogramsecond_;
  private static Unit_ displayUnit_ = perkilogramsecond_;
#endregion

#region comparisons

  public override int GetHashCode() => base.GetHashCode();
  public override bool Equals(Object obj) => obj is PerMassTime other && Equals(other);
  public bool Equals(PerMassTime other) => other is object && Math.Abs(Value_ - other.Value_) <= epsilon;
...
#region conversions
  public static PerMassTime FromPerKilogramSeconds(double value, double minValue, double maxValue) => new PerMassTime((value + perkilogramsecond_.Offset) * perkilogramsecond_.Factor, minValue * perkilogramsecond_.Factor, maxValue * perkilogramsecond_.Factor);
  public static PerMassTime FromPerKilogramSeconds(double value) => new PerMassTime((value + perkilogramsecond_.Offset) * perkilogramsecond_.Factor);
  public double GetValueInPerKilogramSeconds() => (Value_ - perkilogramsecond_.Offset) / perkilogramsecond_.Factor;
...
#region rules
  ...
  public bool ValidateRules(bool throwOnFail=true)
  {
    ...
    if (fail && throwOnFail)
    {
      throw new System.ArgumentException("PerMassTime is outside its bounds.");
    }
    else if (fail)
    {
      return false;
