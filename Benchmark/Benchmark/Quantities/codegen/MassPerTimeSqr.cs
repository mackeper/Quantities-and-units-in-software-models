...
#region units
  private static Unit_ kilogrampersecondsqr_ = new Unit_("KilogramPerSecondSqr", "kg/s²", 1.0, 0);
...
#region comparisons
  ...
  public override bool Equals(Object obj) => obj is MassPerTimeSqr other && Equals(other);
  public bool Equals(MassPerTimeSqr other) => other is object && Math.Abs(Value_ - other.Value_) <= epsilon;
...
#region conversions
  public static MassPerTimeSqr FromKilogramPerSecondSqrs(double value, double minValue, double maxValue) => new MassPerTimeSqr((value + kilogrampersecondsqr_.Offset) * kilogrampersecondsqr_.Factor, minValue * kilogrampersecondsqr_.Factor, maxValue * kilogrampersecondsqr_.Factor);
  public static MassPerTimeSqr FromKilogramPerSecondSqrs(double value) => new MassPerTimeSqr((value + kilogrampersecondsqr_.Offset) * kilogrampersecondsqr_.Factor);
  public double GetValueInKilogramPerSecondSqrs() => (Value_ - kilogrampersecondsqr_.Offset) / kilogrampersecondsqr_.Factor;
...
#region rules
  ...
  public bool ValidateRules(bool throwOnFail=true)
  {
    ...
    if (fail && throwOnFail)
    {
      throw new System.ArgumentException("MassPerTimeSqr is outside its bounds.");
    }
    else if (fail)
    {
      return false;
