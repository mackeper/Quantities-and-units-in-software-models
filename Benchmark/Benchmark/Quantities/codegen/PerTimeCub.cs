...
namespace RayQuantities
{
  ...
  {  
    ...
    if (!displayUnitIsImported_ && FakeDatabase.Units.ContainsKey("PerTimeCub"))
    {
      ...
      {
        case "persecondcub": displayUnit_ = persecondcub_; break;
...
#region units
  private static Unit_ persecondcub_ = new Unit_("PerSecondCub", "/s³", 1.0, 0);
  private static Unit_ platformUnit_ = persecondcub_;
  private static Unit_ displayUnit_ = persecondcub_;
#endregion

#region comparisons

  public override int GetHashCode() => base.GetHashCode();
  public override bool Equals(Object obj) => obj is PerTimeCub other && Equals(other);
  public bool Equals(PerTimeCub other) => other is object && Math.Abs(Value_ - other.Value_) <= epsilon;
...
#region conversions
  public static PerTimeCub FromPerSecondCubs(double value, double minValue, double maxValue) => new PerTimeCub((value + persecondcub_.Offset) * persecondcub_.Factor, minValue * persecondcub_.Factor, maxValue * persecondcub_.Factor);
  public static PerTimeCub FromPerSecondCubs(double value) => new PerTimeCub((value + persecondcub_.Offset) * persecondcub_.Factor);
  public double GetValueInPerSecondCubs() => (Value_ - persecondcub_.Offset) / persecondcub_.Factor;
...
#region rules
  ...
  public bool ValidateRules(bool throwOnFail=true)
  {
    ...
    if (fail && throwOnFail)
    {
      throw new System.ArgumentException("PerTimeCub is outside its bounds.");
    }
    else if (fail)
    {
      return false;
