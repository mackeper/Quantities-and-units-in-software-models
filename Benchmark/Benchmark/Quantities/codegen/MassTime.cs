...
#region units
  private static Unit_ kilogramsecond_ = new Unit_("KilogramSecond", "kg s", 1, 0);
...
#region comparisons
  ...
  public override bool Equals(Object obj) => obj is MassTime other && Equals(other);
  public bool Equals(MassTime other) => other is object && Math.Abs(Value_ - other.Value_) <= epsilon;
...
#region conversions
  public static MassTime FromKilogramSeconds(double value, double minValue, double maxValue) => new MassTime((value + kilogramsecond_.Offset) * kilogramsecond_.Factor, minValue * kilogramsecond_.Factor, maxValue * kilogramsecond_.Factor);
  public static MassTime FromKilogramSeconds(double value) => new MassTime((value + kilogramsecond_.Offset) * kilogramsecond_.Factor);
  public double GetValueInKilogramSeconds() => (Value_ - kilogramsecond_.Offset) / kilogramsecond_.Factor;
...
#region rules
  ...
  public bool ValidateRules(bool throwOnFail=true)
  {
    ...
    if (fail && throwOnFail)
    {
      throw new System.ArgumentException("MassTime is outside its bounds.");
    }
    else if (fail)
    {
      return false;
