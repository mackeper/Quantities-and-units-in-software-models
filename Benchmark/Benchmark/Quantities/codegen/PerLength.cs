...
namespace RayQuantities
{
  ...
  {  
    ...
    if (!displayUnitIsImported_ && FakeDatabase.Units.ContainsKey("PerLength"))
    {
      ...
      {
        case "percentimeter": displayUnit_ = percentimeter_; break;
        case "permillimeter": displayUnit_ = permillimeter_; break;
        case "permeter": displayUnit_ = permeter_; break;
        case "perinch": displayUnit_ = perinch_; break;
...
#region units
  private static Unit_ percentimeter_ = new Unit_("PerCentimeter", "/cm", 1.0, 0);
  private static Unit_ permillimeter_ = new Unit_("PerMillimeter", "/mm", 0.1, 0);
  private static Unit_ permeter_ = new Unit_("PerMeter", "/m", 100.0, 0);
  private static Unit_ perinch_ = new Unit_("PerInch", "/in", 2.56, 0);
  private static Unit_ platformUnit_ = percentimeter_;
  private static Unit_ displayUnit_ = percentimeter_;
#endregion

#region comparisons

  public override int GetHashCode() => base.GetHashCode();
  public override bool Equals(Object obj) => obj is PerLength other && Equals(other);
  public bool Equals(PerLength other) => other is object && Math.Abs(Value_ - other.Value_) <= epsilon;
...
#region conversions
  public static PerLength FromPerCentimeters(double value, double minValue, double maxValue) => new PerLength((value + percentimeter_.Offset) * percentimeter_.Factor, minValue * percentimeter_.Factor, maxValue * percentimeter_.Factor);
  public static PerLength FromPerCentimeters(double value) => new PerLength((value + percentimeter_.Offset) * percentimeter_.Factor);
  public double GetValueInPerCentimeters() => (Value_ - percentimeter_.Offset) / percentimeter_.Factor;
  public static PerLength FromPerMillimeters(double value, double minValue, double maxValue) => new PerLength((value + permillimeter_.Offset) * permillimeter_.Factor, minValue * permillimeter_.Factor, maxValue * permillimeter_.Factor);
  public static PerLength FromPerMillimeters(double value) => new PerLength((value + permillimeter_.Offset) * permillimeter_.Factor);
  public double GetValueInPerMillimeters() => (Value_ - permillimeter_.Offset) / permillimeter_.Factor;
  public static PerLength FromPerMeters(double value, double minValue, double maxValue) => new PerLength((value + permeter_.Offset) * permeter_.Factor, minValue * permeter_.Factor, maxValue * permeter_.Factor);
  public static PerLength FromPerMeters(double value) => new PerLength((value + permeter_.Offset) * permeter_.Factor);
  public double GetValueInPerMeters() => (Value_ - permeter_.Offset) / permeter_.Factor;
  public static PerLength FromPerInchs(double value, double minValue, double maxValue) => new PerLength((value + perinch_.Offset) * perinch_.Factor, minValue * perinch_.Factor, maxValue * perinch_.Factor);
  public static PerLength FromPerInchs(double value) => new PerLength((value + perinch_.Offset) * perinch_.Factor);
  public double GetValueInPerInchs() => (Value_ - perinch_.Offset) / perinch_.Factor;
...
#region rules
  ...
  public bool ValidateRules(bool throwOnFail=true)
  {
    ...
    if (fail && throwOnFail)
    {
      throw new System.ArgumentException("PerLength is outside its bounds.");
    }
    else if (fail)
    {
      return false;
